...
class Solution:
    ...
    def M_y_prime(self,x):
        sol = self.sol
        return -sol["Fz_1"]*step(x,self.geo.x_1,power=1)-sol["Fz_2"]*step(x,self.geo.x_2,power=1)-sol["Fz_3"]*step(x,self.geo.x_3,power=1)\
            -sol["Fa"]*self.case.a_z*step(x,self.case.x_I,power=1)-self.case.P*self.case.a_z*step(x,self.case.x_II,power=1)
